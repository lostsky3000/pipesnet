...
static int init_lua_state(struct lua_service_context* ctx)
{
	...
	do
	{
		...
		*/
		...
		//
		const char *path = optstring("lua_path", "./adapter/lua/lib/?.lua");
		lua_pushstring(L, path);
		lua_setglobal(L, "LUA_PATH");
		const char *cpath = optstring("lua_cpath", "./adapter/lua/clib/?.so");
		...
		//
		const char * loader = optstring("lua_loader", "./adapter/lua/lib/lpps_service_loader.lua");
...
void* luapps_on_boot_create(int curThread)
{
	...
	const char* bootService = lua_pipes_getenv("boot_service");
