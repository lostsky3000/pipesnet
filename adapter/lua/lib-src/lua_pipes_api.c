...
static int l_localid(lua_State* L)
{
	uint64_t id = luaL_checkinteger(L, 1);
	uint32_t idLocal = pipes_handle_id2local(id);
	lua_pushinteger(L, idLocal);
	return 1;
}
static int l_local2id(lua_State* L)
{
	struct lua_service_context* ctx = lua_touserdata(L, lua_upvalueindex(1));
	uint64_t localId = luaL_checkinteger(L, 1);
	uint64_t id = pipes_handle_local2id(localId, ctx->pipes_ctx->thread->global->harbor);
